# Regolith - Project Specification (snapshot 2026-07-21, status refreshed 2026-08-20)
...
**Read §7 before trusting §§2-6.** The body of this document is a 2026-07-21
snapshot and parts of it have been overtaken: the terrain is now drawn from a
mesh rather than a heightmap image, an RGB-D visual odometry package exists
(and is off by default because it measured worse), and M4's acceptance is not
reliably met. §7 was refreshed on 2026-08-20 and says which claims changed;
`PROGRESS.md` is authoritative where the two disagree.
...
It is a **seed demo** for the open-source Regolith project, positioned as a
genuine fork of [Autoware](https://autoware.org/) adapted for GPS-denied,
rough-terrain rover autonomy rather than road-driving cars. Milestones M0-M3
and M5 defined in `PLAN.md` are done or substantially done; **M4's arrival
accuracy is not reliably met**. See §7 for exactly what's still open.
...
- A guaranteed-clear circular spawn zone (`spawn_zone_radius_m=12`, default
  centred at origin) kept free of craters/rocks.
...
Recovery behaviour per `PLAN.md`: stop-and-flag on deviation/stall, no
elaborate FDIR.
...
caveat, this is where the "snapped to grid" behaviour comes from).
...
isolated localisation-pipeline measurements (manual `/cmd_vel`, autonomy
...
both log clearly labelled recovery actions, but they catch different failure
modes and use different recovery mechanisms. (Terminology note: the flip
detector's own progressive-backoff-on-relapse behaviour is informally called
...
labelled a *simulated* recovery in every log line - a wheeled rover cannot
...
flip case, this is *not* labelled simulated - a real rover's FDIR could
...
1. ~~M3 position drift (20-45%, target <5%)~~ **Retracted 2026-07-21**: this
   ...
   collision-seam artefact). **Since measured during full autonomous runs**
   (2026-08): 0.4-0.7% of distance on seeds 42 and 7, but 5-11% on seed 123,
   and repeats of one seed vary by an order of magnitude. The isolated-leg
   figure holds; the full-run figure does not always.
2. **A rare, intermittent wheels-locked-but-upright stall in tight turns**
   ...
   its detection logic is unit-tested. **This is now validated
   end-to-end**: across the nine recorded runs of the 2026-08 acceptance
   campaign the detector fired 61 times and the escape freed the rover 61
   times, 2 to 9 times per run. What is *not* explained is a separate stall
   mode found on the tour mission - see item 9.
3. **Terrain-collision flip risk on long autonomous runs** - root cause is
   ...
   deliberately induced flip). **Flips are no longer observed at all**: zero
   across every acceptance run since the terrain-collision fix, including
   the nine recorded runs of the 2026-08 campaign. Note that the "passed
   3/3" claim this item used to carry is **retracted** - that run happened
   on a world where rock collision was a silent no-op, so the rover drove
   through the boulders. See item 10 for where arrival accuracy actually
   stands.
4. **Goal arrival tolerance is grid-snapped, not exact**: `/goal_reached`
   ...
   the goal snapped to the nearest costmap cell centre (0.781 m/cell at
   ...
   in all three cases. **Partly addressed**: acceptance is now judged by
   `scripts/m4_acceptance.py` on `/ground_truth/pose` against the raw
   commanded goal, never on `/goal_reached`, and the follower has a bounded
   terminal-approach phase (`TerminalApproach`) so its stopping tolerance can
   be tightened without it circling a goal it cannot quite reach. The
   tolerance itself is still at the shipped 1.0 m pending more replicates -
   see item 10.
...
9. **Check both repositories' push state separately.** `src/` is gitignored by
   the meta-repo, so `regolith.universe`'s commits, working-tree changes and
   unpushed history are **completely invisible to `git status` run from the
   meta-repo root**. This has bitten once already: real code changes sat
   uncommitted in the packages repo for a full session while only the
   meta-repo's docs describing them got committed, because the meta-repo
   looked clean. `cd src/regolith.universe && git status` is a separate
   question from `git status`, and both need asking.
10. **An undiagnosed traction failure on benign ground** (2026-08). On the tour
    mission the rover stalled three times *inside* the rock-free spawn zone:
    costmap cost 8 out of 100, nearest boulder 6.7 m, no collision seam within
    1.4 m, driving straight rather than turning, wheels claiming 2.76 m while
    the body moved 0.00 m and attitude spanned 0.18 deg. That is neither the
    boulder wedging of item 2 nor the tight-turn friction lock of §6. **Not
    root-caused.** Recorded with its evidence in `PROGRESS.md`, "Retraction:
    the inflation lever does nothing, and the stall is not an obstacle".
11. **M4 arrival accuracy, and the variance underneath it** (2026-08). One
    seed of three finishes inside the 1.5 m bar, and which seed does is not
    stable: seed 7 passes at 1.46-1.47 m with `goal_tolerance_m` tightened to
    0.35 m and fails at 1.53-1.55 m with the shipped 1.0 m; seed 42 has
    produced a 1.50 m pass and 5.69 / 7.76 m failures on the same build and
    goal; seed 123 is drift-limited at 10.3-10.5 m. Run-to-run drift variance
    is larger than any control parameter is worth, so single-run scores are
    not measurements. The replicate campaign is unfinished. See `PROGRESS.md`,
    "The stopping tolerance, measured".
12. **Visual odometry exists and is off by default** (2026-08).
    `regolith_visual_odometry` (RGB-D, publishes body-frame velocity, EKF
    fuses `vy` only) is unbiased in isolation but measured *worse* in a
    same-build controlled comparison on all three seeds. Enable with
    `visual_odometry:=true` only to work on it. Sections 2-6 of this document
    predate it.
13. **The logo referenced by `README.md` is not in the repository.**
    `docs/assets/regolith_logo_slogan.png` is missing, so the README's header
    image is broken on GitHub.
